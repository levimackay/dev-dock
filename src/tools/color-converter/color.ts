...
export function parseColor(input: string): ParsedColor | null {
  ...
  if (name.startsWith('rgb')) {
    ...
    return {
      format: 'rgb',
      rgb: { r: clamp(Math.round(r), 0, 255), g: clamp(Math.round(g), 0, 255), b: clamp(Math.round(b), 0, 255), a: alpha },
    }
  }
  ...
  if (name.startsWith('hsl')) {
    ...
    return { format: 'hsl', rgb: hslToRgb({ h, s: clamp(s, 0, 100), l: clamp(l, 0, 100), a: alpha }) }
  }
